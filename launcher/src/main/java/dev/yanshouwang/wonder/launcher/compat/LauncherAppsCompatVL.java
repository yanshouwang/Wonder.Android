...
import dev.yanshouwang.wonder.launcher.testing.TestProtocol;
...
public class LauncherAppsCompatVL extends LauncherAppsCompat {
    ...
    public ApplicationInfo getApplicationInfo(String packageName, int flags, UserHandle user) {
        ...
        if (!isPrimaryUser && (flags == 0)) {
            // We are looking for an installed layout_app on a secondary profile. Prior to O, the only
        ...
        try {
            ...
            // There is no way to check if the layout_app is installed for managed profile. But for
    ...
    private static class WrappedCallback extends LauncherApps.Callback {
        ...
        public void onPackagesSuspended(String[] packageNames, UserHandle user) {
            if (TestProtocol.sDebugTracing) {
                Log.d(TestProtocol.APP_NOT_DISABLED, "onPackagesSuspended: " + Arrays.toString(packageNames));
            }
...
//    }
    ...
    public void registerSessionCallback(LooperExecutor executor, SessionCallback sessionCallback) {
        mContext.getPackageManager().getPackageInstaller().registerSessionCallback(sessionCallback, executor.getHandler());
    }

    @Override
    public void unregisterSessionCallback(SessionCallback sessionCallback) {
        mContext.getPackageManager().getPackageInstaller().unregisterSessionCallback(sessionCallback);
    }
}
